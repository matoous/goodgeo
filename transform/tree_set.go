...
type Compare interface {
	IsEquals(x, y goodgeo.Coord) bool
	IsLess(x, y goodgeo.Coord) bool
}
...
func NewTreeSet(layout goodgeo.Layout, compare Compare) *TreeSet {
	return &TreeSet{
		layout:  layout,
...
func (set *TreeSet) Insert(coord goodgeo.Coord) bool {
	if set.stride == 0 {
...
func (set *TreeSet) insertImpl(t *tree, v []float64) (*tree, bool) {
	...
	if set.compare.IsEquals(goodgeo.Coord(v), t.value) {
		return t, false
	}

	var added bool
	if set.compare.IsLess(goodgeo.Coord(v), t.value) {
