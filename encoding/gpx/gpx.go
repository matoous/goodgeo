...
func NewRteType(g *goodgeo.LineString) *RteType {
	return &RteType{
		RtePt: newWptTypes(g),
	}
}
...
func (r *RteType) Geom(layout goodgeo.Layout) *goodgeo.LineString {
	...
	return goodgeo.NewLineStringFlat(layout, flatCoords)
}

// NewTrkType returns a new TrkType with geometry g.
func NewTrkType(g *goodgeo.MultiLineString) *TrkType {
...
func (t *TrkType) Geom(layout goodgeo.Layout) *goodgeo.MultiLineString {
	...
	return goodgeo.NewMultiLineStringFlat(layout, flatCoords, ends)
}

// NewTrkSegType returns a new TrkSegType with geometry g.
func NewTrkSegType(g *goodgeo.LineString) *TrkSegType {
	return &TrkSegType{
		TrkPt: newWptTypes(g),
	}
}
...
func (ts *TrkSegType) Geom(layout goodgeo.Layout) *goodgeo.LineString {
...
func NewWptType(g *goodgeo.Point) *WptType {
...
func (w *WptType) Geom(layout goodgeo.Layout) *goodgeo.Point {
	return goodgeo.NewPointFlat(layout, w.appendFlatCoords(make([]float64, 0, layout.Stride()), layout))
}
...
func (w *WptType) appendFlatCoords(flatCoords []float64, layout goodgeo.Layout) []float64 {
	switch layout {
...
func newWptTypes(g *goodgeo.LineString) []*WptType {
	flatCoords := g.FlatCoords()
	layout := g.Layout()
