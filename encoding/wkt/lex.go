...
func (l *wktLex) validateAndSetLayoutIfNoLayout(layout goodgeo.Layout) bool {
...
func (l *wktLex) validateBaseTypeEmptyAllowed() bool {
	...
	case goodgeo.NoLayout:
		l.setLayoutIfNoLayout(goodgeo.XY)
		fallthrough
	case goodgeo.XY:
		return true
	default:
		l.setIncorrectLayoutError(goodgeo.XY, "EMPTY is XY layout in base geometry type")
		return false
	}
}

// validateAndPushLayoutStackFrame validates that a given layout is valid and pushes a frame to the layout stack.
func (l *wktLex) validateAndPushLayoutStackFrame(layout goodgeo.Layout) bool {
	...
	if layout != goodgeo.NoLayout && !isCompatibleLayout(l.curLayout(), layout) {
		l.setIncorrectLayoutError(layout, "")
		return false
	}
...
func (l *wktLex) setLayoutIfNoLayout(layout goodgeo.Layout) {
	if l.curLayout() == goodgeo.NoLayout {
...
func (l *wktLex) setIncorrectLayoutError(incorrectLayout goodgeo.Layout, hint string) {
...
func (l *wktLex) curLayout() goodgeo.Layout {
...
func isValidStrideForLayout(stride int, layout goodgeo.Layout) bool {
	switch layout {
	case goodgeo.NoLayout:
		return true
	case goodgeo.XY:
	...
	default:
		...
		panic(fmt.Sprintf("unknown goodgeo.Layout %d", layout))
	}
}

// defaultLayoutForStride returns the default layout for a base type geometry with the given stride.
func defaultLayoutForStride(stride int) goodgeo.Layout {
	switch stride {
	case 2:
		return goodgeo.XY
	case 3:
...
func isCompatibleLayout(outerLayout, innerLayout goodgeo.Layout) bool {
	assertValidLayout(outerLayout)
	assertValidLayout(innerLayout)
	if outerLayout != innerLayout && outerLayout != goodgeo.NoLayout {
		return false
	}
	return true
}
...
func layoutName(layout goodgeo.Layout) string {
	switch layout {
	// goodgeo.NoLayout is used when a base type geometry is read.
	case goodgeo.NoLayout:
		return "not XYM"
	case goodgeo.XY:
		return "XY"
...
func assertValidLayout(layout goodgeo.Layout) {
	switch layout {
	case goodgeo.NoLayout, goodgeo.XY, goodgeo.XYM, goodgeo.XYZ, goodgeo.XYZM:
		return
	default:
		panic(fmt.Sprintf("unknown goodgeo.Layout %d", layout))
	}
