...
func makeLayoutStack() layoutStack {
	return layoutStack{
		data: []layoutStackObj{{layout: goodgeo.NoLayout, inBaseTypeCollection: true}},
	}
}

// push constructs a layoutStackObj for a layout and pushes it onto the layout stack.
func (s *layoutStack) push(layout goodgeo.Layout) {
	...
	case goodgeo.XYM, goodgeo.XYZ, goodgeo.XYZM:
...
func (s *layoutStack) pop() goodgeo.Layout {
	s.assertNotEmpty()
	if s.atTopLevel() {
...
func (s *layoutStack) topLayout() goodgeo.Layout {
	return s.top().layout
}
...
func (s *layoutStack) setTopLayout(layout goodgeo.Layout) {
	switch layout {
	case goodgeo.XY, goodgeo.XYM, goodgeo.XYZ, goodgeo.XYZM:
		s.top().layout = layout
	case goodgeo.NoLayout:
		panic("setTopLayout should not be called with goodgeo.NoLayout")
	default:
		// This should never happen.
		panic(fmt.Sprintf("unknown goodgeo.Layout %d", layout))
	}
