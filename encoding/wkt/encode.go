...
func (e *Encoder) Encode(g goodgeo.T) (string, error) {
	sb := &strings.Builder{}
...
func (e *Encoder) write(sb *strings.Builder, g goodgeo.T) error {
	var typeString string
	switch g := g.(type) {
	case *goodgeo.Point:
		typeString = tPoint
	case *goodgeo.LineString, *goodgeo.LinearRing:
	...
	case goodgeo.NoLayout:
		...
		if g, ok := g.(*goodgeo.GeometryCollection); !ok || !g.Empty() {
			return goodgeo.UnsupportedLayoutError(layout)
		}
	case goodgeo.XY:
	case goodgeo.XYZ:
		typeString += tZ
